import { translateStatement } from '../../../../../../src/query/builder/find/statement/translateStatement';
...
describe('translateStatement', () => {
  const testCases: [string, Record<string, string>, string][] = [
    ...
    ['abc(.def.ghi:abc)', { '@': 'n0' }, 'abc(n0.def.ghi:abc)'],
  ...
  test.each(testCases)(
    ...
    (statement: string, entries: Record<string, string>, expected: string) => {
      expect(
        translateStatement(
          statement,
          new VariableSyntaxTranslator(
            new Map(Object.entries(entries).map(([key, v]) => [key, [v, null]]))
          )
        )
      ).toBe(expected);
    }
